import importlib
import json


def test_stop_loss(tmp_path, monkeypatch):
    account_path = tmp_path / "account.json"

    account = {
        "balance": 100.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    paper_trader.open_trade("ETHUSD", 2500.00)
    paper_trader.check_trade(2475.00)

    final_account = json.loads(account_path.read_text())

    assert final_account["balance"] == 99.50
    assert final_account["trades"] == 1
    assert final_account["wins"] == 0
    assert final_account["losses"] == 1
    assert final_account["total_profit"] == -0.50
    assert final_account["daily_loss"] == 0.50
    assert final_account["position"] is None


def test_take_profit(tmp_path, monkeypatch):
    account_path = tmp_path / "account.json"

    account = {
        "balance": 100.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    paper_trader.open_trade("ETHUSD", 2500.00)
    paper_trader.check_trade(2550.00)

    final_account = json.loads(account_path.read_text())

    assert final_account["balance"] == 101.00
    assert final_account["trades"] == 1
    assert final_account["wins"] == 1
    assert final_account["losses"] == 0
    assert final_account["total_profit"] == 1.00
    assert final_account["position"] is None


def test_position_persists_after_open(tmp_path, monkeypatch):
    account_path = tmp_path / "account.json"

    account = {
        "balance": 100.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    paper_trader.open_trade("ETHUSD", 2500.00)

    saved_account = json.loads(account_path.read_text())

    assert saved_account["position"] is not None
    assert saved_account["position"]["pair"] == "ETHUSD"
    assert saved_account["position"]["entry"] == 2500.00
    assert saved_account["position"]["amount"] == 50


def test_insufficient_balance_blocks_trade(tmp_path, monkeypatch):

    account_path = tmp_path / "account.json"

    account = {
        "balance": 25.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    paper_trader.open_trade("ETHUSD", 2500.00, trade_size=50)

    final_account = json.loads(account_path.read_text())

    assert final_account["balance"] == 25.00
    assert final_account["position"] is None


def test_duplicate_position_is_blocked(tmp_path, monkeypatch):

    account_path = tmp_path / "account.json"

    account = {
        "balance": 100.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    paper_trader.open_trade("ETHUSD", 2500.00, trade_size=50)

    first_position = paper_trader.position.copy()

    paper_trader.open_trade("ETHUSD", 2600.00, trade_size=50)

    final_account = json.loads(account_path.read_text())

    assert final_account["balance"] == 50.00
    assert final_account["position"] == first_position
    assert final_account["position"]["entry"] == 2500.00


def test_zero_trade_size_is_blocked(tmp_path, monkeypatch):

    account_path = tmp_path / "account.json"

    account = {
        "balance": 100.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    paper_trader.open_trade("ETHUSD", 2500.00, trade_size=0)

    final_account = json.loads(account_path.read_text())

    assert final_account["balance"] == 100.00
    assert final_account["position"] is None


def test_account_state_persists_after_close(tmp_path, monkeypatch):

    account_path = tmp_path / "account.json"

    account = {
        "balance": 100.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    paper_trader.open_trade("ETHUSD", 2500.00, trade_size=50)
    paper_trader.check_trade(2550.00)

    saved_account = json.loads(account_path.read_text())

    assert saved_account["balance"] == 101.00
    assert saved_account["trades"] == 1
    assert saved_account["wins"] == 1
    assert saved_account["losses"] == 0
    assert saved_account["total_profit"] == 1.00
    assert saved_account["position"] is None


def test_check_trade_without_position_does_nothing(tmp_path, monkeypatch):

    account_path = tmp_path / "account.json"

    account = {
        "balance": 100.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    paper_trader.check_trade(2500.00)

    final_account = json.loads(account_path.read_text())

    assert final_account["balance"] == 100.00
    assert final_account["trades"] == 0
    assert final_account["wins"] == 0
    assert final_account["losses"] == 0
    assert final_account["total_profit"] == 0.00
    assert final_account["position"] is None


def test_risk_manager_rejection_blocks_trade(tmp_path, monkeypatch):

    account_path = tmp_path / "account.json"

    account = {
        "balance": 100.00,
        "trades": 0,
        "wins": 0,
        "losses": 0,
        "total_profit": 0.00,
        "position": None,
        "daily_loss": 0.00,
    }

    account_path.write_text(json.dumps(account))

    monkeypatch.chdir(tmp_path)

    import paper_trader
    importlib.reload(paper_trader)

    monkeypatch.setattr(paper_trader, "record_trade", lambda: False)

    paper_trader.open_trade("ETHUSD", 2500.00, trade_size=50)

    final_account = json.loads(account_path.read_text())

    assert final_account["balance"] == 100.00
    assert final_account["position"] is None
